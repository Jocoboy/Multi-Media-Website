...
function saveMySpecialCard() {
    ...
    for (let i = 0; i < cardcontainerElement.childNodes.length; i++) {
        if (cardcontainerElement.childNodes[i].classList.contains('scanned')) {
            var name = cardcontainerElement.childNodes[i].style.backgroundImage.slice(20, -6);
...
function showMySpecialCard() {
    for (let i = 0; i < mySpecialCardList.length; i++) {
        ...
        card.style.backgroundImage = 'url(../images/card/' + mySpecialCardList[i][0] + '.jpg)';
...
function reflowOwnedCard(ownedcardList) {
    ...
    for (let i = 0; i < ownedcardList.length; i++) {
        var card = document.createElement('div');
        card.style.backgroundImage = 'url(../images/card/' + ownedcardList[i] + '.jpg)';
        card.classList.add('scanned');
        viewportcontainerElement.appendChild(card);
    }
...
function createCardBack(num, hd, parent) {
    for (let i = 0; i < num; i++) {
        ...
        cardBack.style.backgroundImage = 'url(../images/' + 'cardback.jpg' + ')';
...
function createNormalCard(num, hd, parent) {
    ...
    if (hd != undefined && parent != undefined) {
        for (let i = 0; i < num; i++) {
            var card = document.createElement('div');
            card.classList.add('card');
            card.style.backgroundImage = 'url(../images/card/' + cards[i] + '.jpg)';
...
function listenNormalCard(element, st = 0) {
    ...
    for (let j = st; j < element.childNodes.length; j++) {
        let cardName = element.childNodes[j].style.backgroundImage.slice(20, -6);
    ...
    for (let i = 0; i < dom.childNodes.length; i++) {
        if (checkSeason()) {
            dom.childNodes[i].onclick = function () {
                ...
                dom.childNodes[i] && (myCardName = dom.childNodes[i].style.backgroundImage.slice(20, -6));
                for (let j = INITIAL_CARDBACK_NUM; j < poolcardsElement.childNodes.length; j++) {
                    var poolCardName = poolcardsElement.childNodes[j].style.backgroundImage.slice(20, -6);
...
function shuffleDeck() {
    ...
    while (poolcardsElement.childNodes.length > INITIAL_CARDBACK_NUM) {
        let name = poolcardsElement.lastChild.style.backgroundImage.slice(20, -6);
...
function swapCards(myCard, poolCard) {
    poolCardName = poolCard.style.backgroundImage.slice(20, -6);
    myCardName = myCard.style.backgroundImage.slice(20, -6);
...
function checkSeason() {
    ...
    for (let i = 0; i < mycardsElement.childNodes.length; i++) {
        var myCardName = mycardsElement.childNodes[i].style.backgroundImage.slice(20, -6);
        for (let j = INITIAL_CARDBACK_NUM; j < poolcardsElement.childNodes.length; j++) {
            var poolCardName = poolcardsElement.childNodes[j].style.backgroundImage.slice(20, -6);
            if (normalCardList.get(myCardName)[2] == normalCardList.get(poolCardName)[2]) {
                return true;
            ...
            if (this.classList.contains('cardshadow')) {
                ...
                for (let j = 0; j < mycardsElement.childNodes.length; j++) {
                    if (mycardsElement.childNodes[j].classList.contains('cardshadow')) {
                        ...
                        var poolCardName = this.style.backgroundImage.slice(20, -6);
                        var myCardName = mycardsElement.childNodes[j].style.backgroundImage.slice(20, -6);
                        ...
                        for (let pos = 2; pos >= 1; pos--) {
                            for (let k = 0; k < specialcard.childNodes.length; k++) {
                                var specialCardName = specialcard.childNodes[k].style.backgroundImage.slice(20, -6);
    ...
    for (let i = 0; i < elements.length; i++) {
        ...
        if (specialCardList != undefined) {
            for (let j = 0; j < specialCardList.length; j++) {
                if (specialCardList[j][0].split('·')[0] == elements[i].style.backgroundImage.slice(20, -6)) {
                    elements[i].style.backgroundImage = 'url(../images/card/' + specialCardList[j][0] + '.jpg)';
                    break;
                }
            }
        }
...
function reflowPoolCards() {
    ...
    var card = document.createElement('div');
    card.classList.add('card');
    card.style.backgroundImage = 'url(../images/card/' + visitedNormalCardList[index][0] + '.jpg)';
    card.style.left = l + '%';
    card.style.bottom = (i & 1) + '%';
...
function simulate() {
    ...
    if (!check) {
        ...
        for (let i = 0; i < poolcardsElement.childNodes.length; i++) {
            var poolCardName = poolcardsElement.childNodes[i].style.backgroundImage.slice(20, -6);
            if (poolCardName == deletePoolCardName) {
                ...
                yourCard.style.backgroundImage = 'url(../images/card/' + deleteYourCardName + '.jpg)';
                var poolCard = document.createElement('div');
                poolCard.classList.add('card');
                poolCard.style.backgroundImage = 'url(../images/card/' + deletePoolCardName + '.jpg)';
        ...
        poolcardsElement.lastChild.style.backgroundImage = 'url(../images/card/' + yourCardName + '.jpg)';
        listenNormalCard(poolcardsElement, INITIAL_CARDBACK_NUM);
        bindPoolCard(INITIAL_CARDBACK_NUM);
        setTimeout("simulate()", 1600);
    }
