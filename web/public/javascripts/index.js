...
function listenSpecialCard() {
    ...
    for (let [key, value] of specialCardList.entries()) {
        ...
        card.style.backgroundImage = 'url(' + "../images/card/" + key + ".jpg" + ')';
        ...
        card.addEventListener('click', function () {
            var selectedCardName = this.style.backgroundImage.slice(20, -6).split('·')[0];
            if (this.parentElement.id == 'cardrepository') {
                if (isCeiling())
                    ...
                    for (let i = 0; i < cardrepository.childNodes.length; i++) {
                        var repositoryCardName = cardrepository.childNodes[i].style.backgroundImage.slice(20, -6).split('·')[0];
